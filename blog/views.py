...
from .models import UniqueView, Comment, Post
...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['post'] = get_object_or_404(
            Post, slug=self.kwargs['slug'], status='published')
        context['post_tags_ids'] = context[
            'post'
        ].tags.values_list('id', flat=True)
        context['similar_posts'] = Post.objects.filter(
            tags__in=context['post_tags_ids'], status='published'
        ).exclude(id=context['post'].id)
        context['similar_posts'] = context[
            'similar_posts'
        ].annotate(same_tags=Count('tags')
                   ).order_by('-same_tags', '-publish')[:4]
        context['comments'] = Comment.objects.filter(
            post=context['post'], active=True).order_by('-created')
        ...
        counter, created = UniqueView.objects.get_or_create(
            post=context['post'], ip=context['ip'])
        return context
