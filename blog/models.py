...
class Post(models.Model):
    ...
    def get_previous_post(self):
        post = get_object_or_404(Post, slug=self.slug, status='published',
        ...
        similar_post = similar_posts.annotate(
            same_tags=Count('tags')).order_by('-same_tags', '-publish')

        if similar_post:
            return similar_post
        else:
            post_ = Post.objects.filter(status='published',
                                        created__lt=self.created)
            if post_:
                return post_
            else:
    ...
    def get_next_post(self):
        post = get_object_or_404(Post, slug=self.slug,
                                 status='published', publish=self.publish)
        post_tags_ids = post.tags.values_list('id', flat=True)
        similar_posts = Post.objects.filter(tags__in=post_tags_ids,
                                            status='published',
